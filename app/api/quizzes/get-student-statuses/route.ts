...
export async function POST(request: NextRequest) {
  try {
    ...
    const { quiz_id, estudiante_ids } = body;

    if (!quiz_id || !Array.isArray(estudiante_ids)) {
      return NextResponse.json(
        { error: 'quiz_id y estudiante_ids (array) son requeridos' },
        { status: 400 }
      );
    }
    ...
    // Obtener estados individuales de los estudiantes con este quiz
    ...
    if (estadosError) {
      ...
      if (errorMessage.includes("Could not find the table") || errorMessage.includes("does not exist")) {
        ...
        return NextResponse.json({
          success: true,
          data: [],
          warning: 'La tabla quizzes_estudiantes no existe. Los estudiantes mostrarán estado global.',
        }, { status: 200 });
      }
