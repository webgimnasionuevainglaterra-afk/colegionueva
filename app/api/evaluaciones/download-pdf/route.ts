...
export async function GET(request: NextRequest) {
  try {
    ...
    // Obtener la evaluación con todas sus preguntas y opciones
    ...
    const cursoId = evaluaciones.materias?.curso_id || evaluaciones.periodos?.materias?.curso_id;
    ...
    const materiaNombre = evaluaciones.materias?.nombre || 'N/A';
    const periodoNombre = evaluaciones.periodos?.nombre || 'N/A';
    const cursoNombre = evaluaciones.materias?.cursos?.nombre || evaluaciones.periodos?.materias?.cursos?.nombre || 'N/A';
    ...
    const htmlContent = `
      ...
      <html lang="es">
        <head>
          ...
          <title>${evaluaciones.nombre}</title>
          <style>
            * {
              margin: 0;
              padding: 0;
        ...
        <body>
          <div class="header">
            <h1>${evaluaciones.nombre}</h1>
            <div class="subtitle">Evaluación de Periodo</div>
            ${evaluaciones.descripcion ? `
              <div class="descripcion">${evaluaciones.descripcion}</div>
            ` : ''}
          </div>
          ...
          <div class="info-grid" style="padding: 20px 0;">
            ...
            <div class="info-row">
              ...
              <div class="info-item value">${new Date(evaluaciones.fecha_inicio).toLocaleDateString('es-ES', { 
                year: 'numeric', 
                month: 'long', 
                day: 'numeric',
                hour: '2-digit',
            ...
            <div class="info-row">
              ...
              <div class="info-item value">${new Date(evaluaciones.fecha_fin).toLocaleDateString('es-ES', { 
                year: 'numeric', 
                month: 'long', 
                day: 'numeric',
                hour: '2-digit',
