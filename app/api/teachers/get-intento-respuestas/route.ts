...
export async function GET(request: NextRequest) {
  try {
    ...
    const [profesorData, adminData] = await Promise.all([
      supabaseAdmin.from('profesores').select('id').eq('id', user.id).single(),
      supabaseAdmin.from('administrators').select('role').eq('id', user.id).eq('role', 'super_admin').single(),
    ]);
    ...
    console.error('Error get-intento-respuestas:', err);
