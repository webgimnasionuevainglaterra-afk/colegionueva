...
function CreateEvaluacionModal({
  ...
  const actualizarPregunta = (index: number, campo: string, valor: any) => {
    ...
    } else if (campo === 'archivo_url') {
      nuevasPreguntas[index].archivo_url = valor;
    }
    setPreguntas(nuevasPreguntas);
  };

  const handleFileUpload = async (preguntaIndex: number, file: File) => {
    setUploadingFiles(prev => ({ ...prev, [preguntaIndex]: true }));
    try {
      const formData = new FormData();
      formData.append('files', file);

      const response = await fetch('/api/contenido/upload-files', {
        method: 'POST',
        body: formData,
      });
      ...
      if (result.urls && result.urls.length > 0) {
        actualizarPregunta(preguntaIndex, 'archivo_url', result.urls[0]);
      ...
      setUploadingFiles(prev => ({ ...prev, [preguntaIndex]: false }));
    }
  };

  const handleFileChange = (preguntaIndex: number, e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      handleFileUpload(preguntaIndex, file);
    }
    // Limpiar el input para permitir seleccionar el mismo archivo de nuevo
    e.target.value = '';
  };

  const eliminarArchivo = (preguntaIndex: number) => {
    if (confirm('¿Estás seguro de que deseas eliminar el archivo adjunto?')) {
      actualizarPregunta(preguntaIndex, 'archivo_url', null);
    }
  };
              ...
              <div key={preguntaIndex} style={{
                  ...
                  <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                    ...
                    <label
                      style={{
                        ...
                        border: 'none',
                        borderRadius: '4px',
                        cursor: uploadingFiles[preguntaIndex] ? 'not-allowed' : 'pointer',
                        fontSize: '0.75rem',
                        display: 'inline-flex',
                        alignItems: 'center',
                        gap: '0.25rem',
                        opacity: uploadingFiles[preguntaIndex] ? 0.6 : 1,
                      }}
                      title="Adjuntar imagen o documento"
                    >
                      {uploadingFiles[preguntaIndex] ? (
                        'Subiendo...'
                      ) : (
                        <>
                      ...
                      <input
                        ...
                        onChange={(e) => handleFileChange(preguntaIndex, e)}
                        disabled={uploadingFiles[preguntaIndex]}
                        style={{ display: 'none' }}
                      />
                    </label>
                    <button
                ...
                {pregunta.archivo_url && (
                  <div style={{ 
                    ...
                    <button
                      type="button"
                      onClick={() => eliminarArchivo(preguntaIndex)}
                      style={{
                        padding: '0.25rem 0.5rem',
                        background: '#ef4444',
                        color: 'white',
